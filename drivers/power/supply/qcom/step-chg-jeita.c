...
struct step_chg_info {
	...
	int			last_jeita_status; // Only use hysteresis when JEITA changes from normal to cool or warm
...
static int get_step_chg_jeita_setting_from_profile(struct step_chg_info *chip)
{
	...
	if (rc < 0) {
		pr_debug("Read fih,step-chg-hysteresis failed, rc=%d\n", rc);
		chip->step_chg_cfg_valid = false;
	}
#endif
	...
	if (rc < 0) {
		pr_debug("Read fih,jeita-hysteresis failed, rc=%d\n", rc);
		chip->sw_jeita_cfg_valid = false;
	}
#endif
...
static int get_val(struct range_data *range, int hysteresis, int current_index,
		...
		int last_jeita_status, int current_jeita_status) // Only use hysteresis when JEITA changes from normal to cool or warm
{
	int i;
...
static int handle_step_chg_config(struct step_chg_info *chip)
{
	...
	//{Only use hysteresis when JEITA changes from normal to cool or warm
...
{
	union power_supply_propval jeita_pval = {0, }; // Only use hysteresis when JEITA changes from normal to cool or warm
	union power_supply_propval pval = {0, };
...
update_time:
	chip->last_jeita_status = jeita_pval.intval; // Only use hysteresis when JEITA changes from normal to cool or warm
